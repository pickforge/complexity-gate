...
pub fn load_config(start: &Path, explicit: Option<&Path>) -> Result<ConfigResolution> {
    ...
    let repo = explicit.map(Path::to_path_buf).or_else(|| nearest_repo_config(start));
...
fn nearest_repo_config(start: &Path) -> Option<PathBuf> {
    ...
    start.ancestors().map(|dir| dir.join(".complexity-gate.json")).find(|path| path.is_file())
}
...
fn merge_file(base: &mut Value, path: &Path) -> Result<()> {
    ...
    let patch: Value = serde_json::from_str(&text).with_context(|| format!("invalid JSON in {}", path.display()))?;
...
fn shallow_merge(base: &mut Value, patch: Value) {
    let (Some(base), Value::Object(patch)) = (base.as_object_mut(), patch) else { return };
    for (key, value) in patch {
        match (base.get_mut(&key), value) {
            (Some(Value::Object(current)), Value::Object(next)) => current.extend(next),
            (_, next) => { base.insert(key, next); }
        }
    }
}

fn validate_keys(value: &Value, path: &Path) -> Result<()> {
    let object = value.as_object().ok_or_else(|| anyhow::anyhow!("config {} must be an object", path.display()))?;
    allowed(object, &["limits", "tests", "ignore", "languages", "hook"], "", path)?;
    nested_keys(object, "limits", &["complexity", "depth", "lines", "params"], path)?;
...
fn nested_keys(root: &Map<String, Value>, key: &str, keys: &[&str], path: &Path) -> Result<()> {
    if let Some(value) = root.get(key) {
        let object = value.as_object().ok_or_else(|| anyhow::anyhow!("{key} in {} must be an object", path.display()))?;
...
fn validate_language_keys(root: &Map<String, Value>, path: &Path) -> Result<()> {
    let Some(value) = root.get("languages") else { return Ok(()) };
    let languages = value.as_object().ok_or_else(|| anyhow::anyhow!("languages in {} must be an object", path.display()))?;
    for (name, value) in languages {
        let object = value.as_object().ok_or_else(|| anyhow::anyhow!("languages.{name} must be an object"))?;
...
fn validate_languages(config: &Config) -> Result<()> {
    const LANGUAGES: &[&str] = &["javascript", "typescript", "svelte", "dart", "rust", "python", "go"];
    for name in config.languages.keys() {
        if !LANGUAGES.contains(&name.as_str()) { bail!("unknown config key `languages.{name}`"); }
    }
    Ok(())
}
...
impl Config {
    pub fn limits_for(&self, language: &str) -> &Limits {
        self.languages.get(language).and_then(|entry| entry.limits.as_ref()).unwrap_or(&self.limits)
    }

    pub fn matcher(patterns: &[String]) -> Result<GlobSet> {
        let mut builder = GlobSetBuilder::new();
        for pattern in patterns { builder.add(Glob::new(pattern)?); }
        Ok(builder.build()?)
    }
}
...
mod tests {
    ...
    fn unknown_nested_key_names_full_path() {
        ...
        let error = load_config(dir.path(), Some(&path)).unwrap_err().to_string();
